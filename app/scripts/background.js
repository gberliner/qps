...
chrome.printerProvider.onGetPrintersRequested.addListener(chromeCallback => {
  ...
  let piArray = [printerInfo]; // pass our virtual printer's printerInfo back to the chrome runtime
...
chrome.printerProvider.onPrintRequested.addListener((printJob, resultCb) => {
...
function checkUrl(url) {
  ...
  if (xhr.status === 404) {
    return {
      success: `ÙRL ${url} available`
    };
  } else {
    return {
      error: `URL ${url} UNAVAILABLE or error getting status`
    };
  }
}
